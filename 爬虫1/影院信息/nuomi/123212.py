...
class GetComments(object):
    ...
    def get_json(self, song_id, offset):
        ...
        :return: json转成的dict
        """
        url = 'http://music.163.com/api/v1/resource/comments/R_SO_4_%s?limit=20&offset=%s' % (
            song_id, offset)
        print(url)
        ...
        for i in range(page):
            ...
            for item in json_dict['comments']:
                ...
                comment_time = time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(item['time'] / 1000))  # 获取评论时间
                comment_info = comment_time + ' ' + \
                    str(liked_count) + ' ' + comment + '\n'
    ...
    def get_songs_id(self, url):
        ...
        # print(html.text)
        songs_name = text.xpath(
            '//div[@id="hotsong-list"]/div[@class="f-cb"]/div/ul//a/text()')
        songs_id = text.xpath(
            '//div[@id="hotsong-list"]/div[@class="f-cb"]/div/ul//a/@href')  # 获取歌曲id
...
if __name__ == '__main__':
    singer_url = 'https://music.163.com/artist?id=8103'  # 记得要去掉#号，太那啥了
    spider = GetComments()
